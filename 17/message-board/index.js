...
mongodb.MongoClient.connect(url, (err, db) => {
  ...
  //**(1) pass request with property 'messages' that points to mongodb collection 'messages'  */
  app.use((req, res, next) => {
    req.messages = db.collection('messages')
    return next()
  })

  //**(2) req.messages effectively is mongo collection.  */
  ...
  //**(3) the mongo collection data is passed to React Component MessageBoard through 'messages' property */
  //**(4) the script passes mongo collection's 'messages' documents from server to browser(client/app.jsx). */
  //* var messages will contains JSON.stringify(docs). The messages variable is used in client/app.jsx or in another words, public/js/bundle.js. */
